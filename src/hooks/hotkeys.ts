...
import { hotkeyBindingsAtom, normalizeKey, settingsOpenAtom } from "@/atoms/hotkeys";
import { currentSrcAtom, isPlayingAtom, muteAtom, volumeAtom } from "@/atoms/player";
import { type ChannelNum, currentRadioAtom, radioChannelsByAreaAtom } from "@/atoms/radio";
    ...
    (e) => {
      ...
      if (preset) {
        ...
        const nearest = bandStations.reduce<(typeof bandStations)[0] | null>(
          (best, s) =>
            !best || Math.abs(s.freq - defaultFreq) < Math.abs(best.freq - defaultFreq) ? s : best,
          null,
        );
        if (nearest) {
          selectRadio({
