...
export function useSteppedBars(): (getBars: () => AnalyzerBarData[]) => AnalyzerBarData[] | null {
  ...
  function processBars(getBars: () => AnalyzerBarData[]): AnalyzerBarData[] | null {
    if (animationMode === "stepped") {
      if (!steppedRef.current) {
        steppedRef.current = new SteppedAnalyzer(steppedInterval, 1.0, 500, 0.3);
      }
