...
import { clearLegacySessionStore, requestPermission } from "@/lib/fileSessionDb";
...
async function hydrateSong(song: Song): Promise<Song> {
  ...
  try {
    ...
    if (common.picture?.[0]) {
      ...
      artwork = URL.createObjectURL(new Blob([new Uint8Array(pic.data)], { type: pic.format }));
    }
  } catch {
    // non-fatal
  }
...
export function FileRestore() {
  ...
  const shouldRestore = hasSession && currentSrc === "file" && isFileSystemAccessSupported;
  ...
  const needsHydration = shouldRestore && allSongs.some((s) => s.handle && !s.url);
  ...
  const { mutate: restore, isPending: isRestoring } = useMutation({
    mutationFn: async () => {
      const hydratedCurrent = currentSong ? await hydrateSong(currentSong) : null;
  ...
  useEffect(() => {
    if (permissionData && !permissionData.granted) {
      ...
      if (!permissionToastId.current) {
        permissionToastId.current = toast("前回のファイルへのアクセスを許可してください", {
          position: "top-right",
          action: {
            label: "許可する",
            onClick: handleAllow,
