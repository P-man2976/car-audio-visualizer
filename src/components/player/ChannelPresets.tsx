...
export function ChannelPresets() {
  ...
  const station = currentRadio?.source === "radiko" ? frequencies?.[currentRadio.id] : undefined;
  ...
  const assignCurrentToChannel = (ch: ChannelNum) => {
    if (!areaId || currentRadio?.source !== "radiko" || currentRadio.frequency == null) {
      return;
    }
        ...
        return (
          <ContextMenu key={ch}>
            <Tooltip>
                ...
                <ContextMenuTrigger asChild>
                  <Button
                    ...
                    className={`size-10 p-0 font-mono text-base font-bold sm:size-10 sm:text-base md:size-12 md:text-lg ${
                      preset ? (isActive ? "bg-gray-500/30 border" : "") : "opacity-30"
                    }`}
              ...
              <TooltipContent>{preset ? preset.stationName : "現在の局を登録"}</TooltipContent>
            </Tooltip>
            <RadioStationContextMenuContent
              stationId={currentRadio?.source === "radiko" ? currentRadio.id : ""}
              stationName={currentRadio?.name ?? ""}
