...
vi.mock("virtua", () => ({
  VList: ({ children, ...props }: { children: React.ReactNode } & Record<string, unknown>) => (
...
describe("QueueSheet", () => {
  test("ファイルモード: 曲キューが空のとき「キューは空です」", async () => {
    ...
    await expect.element(page.getByRole("tab", { name: "再生待ち" })).toBeInTheDocument();
  ...
  test("ファイルモード: 履歴が空のとき「履歴はありません」", async () => {
    ...
    await page.getByRole("button", { name: "キュー" }).click();
    await page.getByRole("tab", { name: "履歴" }).click();
    await expect.element(page.getByText("履歴はありません")).toBeInTheDocument();
  });
