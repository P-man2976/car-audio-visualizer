...
describe("SettingsDialog", () => {
  ...
  test("4 つのナビゲーションタブが表示される", async () => {
    ...
    await expect.element(page.getByRole("tab", { name: "Last.fm" })).toBeInTheDocument();
    await expect.element(page.getByRole("tab", { name: "ショートカット" })).toBeInTheDocument();
  });
  ...
  test("ビジュアライザーペインでスタイル選択カードが表示される", async () => {
    ...
    await expect
      ...
      .toBeInTheDocument();
    await expect.element(page.getByRole("button", { name: /DPX-5021M（2D）/ })).toBeInTheDocument();
  });
