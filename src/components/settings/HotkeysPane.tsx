    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-neutral-400 leading-relaxed">
          キーをクリックして新しいショートカットを割り当てます。Esc でキャンセル。
        </p>
        <button
          type="button"
        ...
        <div key={section.label} className="flex flex-col gap-1">
          ...
          {section.actions.map((action) => (
            <div key={action} className="flex items-center justify-between py-0.5">
              <span className="text-sm text-neutral-300">{HOTKEY_ACTION_LABELS[action]}</span>
              <button
                type="button"
                className={`
              ...
              >
                {capturing === action ? "キーを押して…" : displayKey(bindings[action] ?? "")}
              </button>
            </div>
          ))}
        </div>
