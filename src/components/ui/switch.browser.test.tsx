...
describe("Switch", () => {
  test("初期状態は OFF (data-state=unchecked)", async () => {
    void render(<Switch aria-label="通知" />);
  ...
  test("onCheckedChange コールバックが呼ばれる", async () => {
    ...
    void render(<Switch aria-label="設定" onCheckedChange={handleChange} />);
  ...
  test("defaultChecked で初期 ON にできる", async () => {
    void render(<Switch aria-label="初期ON" defaultChecked />);
