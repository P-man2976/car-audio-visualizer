...
describe("Sheet", () => {
  ...
  test("showCloseButton でクローズボタンが表示される", async () => {
    ...
    await expect.element(page.getByText("クローズテスト")).not.toBeInTheDocument();
  });
  ...
  test("SheetClose で明示的にシートを閉じられる", async () => {
    ...
    await expect.element(page.getByText("閉じるテスト")).not.toBeInTheDocument();
  });
  ...
  test("open prop で制御モードで動作する", async () => {
    ...
    await expect.element(page.getByText("制御モードシート")).toBeInTheDocument();
  });
});
