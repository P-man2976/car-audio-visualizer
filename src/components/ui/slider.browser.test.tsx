...
describe("Slider", () => {
  ...
  test("disabled で data-disabled 属性が付与される", async () => {
    void render(<Slider defaultValue={[50]} disabled />);
    const slider = page.getByRole("slider");
  ...
  test("onValueChange が呼ばれる（キーボード操作）", async () => {
    ...
    void render(
      <Slider defaultValue={[50]} min={0} max={100} step={1} onValueChange={handleChange} />,
    );
  ...
  test("data-slot=slider がルート要素に設定される", async () => {
    void render(<Slider defaultValue={[50]} data-testid="slider-root" />);
