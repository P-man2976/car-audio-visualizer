...
describe("ContextMenu", () => {
  test("右クリックでメニュー表示、onSelect、disabled、Label/Separator、variant を検証", async () => {
    ...
    await expect.element(page.getByText("無効アイテム")).toHaveAttribute("data-disabled");

    // variant=destructive で data-variant が設定される
    await expect.element(page.getByText("削除")).toHaveAttribute("data-variant", "destructive");
