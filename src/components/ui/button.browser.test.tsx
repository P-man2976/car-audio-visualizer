...
describe("Button", () => {
  test("テキストを正しくレンダリングする", async () => {
    void render(<Button>クリック</Button>);
    await expect.element(page.getByRole("button", { name: "クリック" })).toBeInTheDocument();
  });

  test("クリックイベントが発火する", async () => {
    const handleClick = vi.fn();
    void render(<Button onClick={handleClick}>送信</Button>);
  ...
  test("variant が data-variant 属性に反映される", async () => {
    void render(<Button variant="destructive">削除</Button>);
  ...
  test("size が data-size 属性に反映される", async () => {
    void render(<Button size="sm">小さいボタン</Button>);
