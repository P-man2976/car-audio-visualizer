...
const BAND_GAP = 1.6;
const BAND_STRIDE = SIDE_UNIT + MAIN_BAR_WIDTH + SIDE_GAP + SIDE_BAR_WIDTH + BAND_GAP;
...
export function VisualizerKenwood() {
  ...
  useFrame(() => {
    ...
    const bars = processBars(() => audioMotionAnalyzer.getBars() as AnalyzerBarData[]);
  ...
  return (
    <group position={[-GRID_CX * SCALE, -PROJ_CY * SCALE + OFFSET_Y, 0]} scale={SCALE}>
...
function KenwoodMainBandMesh({ fi }: { fi: number }) {
  ...
  const { geometry, colorArray } = useMemo(() => {
    ...
    for (let ci = 0; ci < COL_CELL_COUNT; ci++) {
      ...
      const lIdx = ci * 2;
      const rIdx = ci * 2 + 1;
      writePerspQuad(positions, lIdx, subLeftCX(fi), y, MAIN_HALF_W, HALF_H, PERSP);
      writePerspQuad(positions, rIdx, subRightCX(fi), y, MAIN_HALF_W, HALF_H, PERSP);
  ...
  useFrame(() => {
    ...
    for (let ci = 0; ci < COL_CELL_COUNT; ci++) {
      ...
      const c = isPeak ? COLOR_PEAK : value * COL_CELL_COUNT > ci ? COLOR_LIT : COLOR_DARK;
...
function KenwoodSideBandMesh({ fi }: { fi: number }) {
  ...
  const { geometry, colorArray } = useMemo(() => {
    ...
    for (let ci = 0; ci < COL_CELL_COUNT; ci++) {
      ...
      const lIdx = ci * 2;
      const rIdx = ci * 2 + 1;
      writePerspQuad(positions, lIdx, sideLeftCX(fi), y, SIDE_HALF_W, HALF_H, PERSP);
      writePerspQuad(positions, rIdx, sideRightCX(fi), y, SIDE_HALF_W, HALF_H, PERSP);
      writeQuadIndices(indices, lIdx);
      writeQuadIndices(indices, rIdx);
      fillQuadColor(colors, lIdx, COLOR_SIDE_OFF.r, COLOR_SIDE_OFF.g, COLOR_SIDE_OFF.b);
      fillQuadColor(colors, rIdx, COLOR_SIDE_OFF.r, COLOR_SIDE_OFF.g, COLOR_SIDE_OFF.b);
    }
