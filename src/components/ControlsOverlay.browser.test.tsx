...
import { currentSongAtom, currentSrcAtom, repeatModeAtom, shuffleAtom } from "@/atoms/player";
...
describe("ControlsOverlay", () => {
  ...
  test("file 状態で曲タイトルとアーティストが表示される", async () => {
    ...
    await expect.element(page.getByTestId("song-title").first()).toHaveTextContent("テスト曲");
    await expect
  ...
  test("radio 状態で局名と FM/AM ボタンが表示される", async () => {
    ...
    await expect.element(page.getByTestId("song-title").first()).toHaveTextContent("TBSラジオ");
  ...
  test("radio 状態で周波数がアーティスト欄に表示される", async () => {
    ...
    await expect.element(page.getByTestId("song-artist").first()).toHaveTextContent("90.5MHz");
  });
  ...
  test("radio 状態でチャンネル番号がバッジとして表示される", async () => {
    ...
    await expect.element(page.getByTestId("song-badge").first()).toHaveTextContent("1");
    await expect.element(page.getByTestId("song-album").first()).toHaveTextContent("Radiko");
  });
  ...
  test("aux 状態で「外部入力」が表示される", async () => {
    ...
    await expect.element(page.getByTestId("song-title").first()).toHaveTextContent("外部入力");
  });
