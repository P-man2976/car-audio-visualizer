...
export function QueueSheet({ children }: { children: ReactNode }) {
  ...
  return (
    <Sheet>
      ...
      >
        <SheetHeader>
          <SheetTitle className="text-xl pl-2">{isRadio ? "最近再生した局" : "キュー"}</SheetTitle>
        </SheetHeader>
...
function SongQueueTabs() {
  return (
    <Tabs defaultValue="queue" className="mt-2 flex-1 flex flex-col min-h-0">
      ...
      <TabsContent value="history" className="flex-1 min-h-0 overflow-hidden flex flex-col">
        <SongHistoryList />
      </TabsContent>
    </Tabs>
  );
...
function SongQueueList() {
  ...
  const repeatCount =
    repeatMode === "all" ? (currentSong ? 1 : 0) + songQueue.length + songHistory.length : 0;
...
function SongHistoryList() {
  ...
  if (!songHistory.length) {
    return <p className="text-sm text-muted-foreground pl-2 mt-4">履歴はありません</p>;
  }
...
function QueueSongCard({ song, context }: { song: Song; context: "queue" | "history" }) {
