...
vi.mock("@/components/source/RadioStation", () => ({
  RadioStation: ({ name }: { name: string }) => <div data-testid="radio-station">{name}</div>,
}));
...
describe("SourceSheet", () => {
  test("Sheet を開くと 3 つのタブ（ファイル/ラジオ/外部入力）が表示される", async () => {
    ...
    await expect.element(page.getByRole("tab", { name: "ファイル" })).toBeInTheDocument();
    await expect.element(page.getByRole("tab", { name: "ラジオ" })).toBeInTheDocument();
    await expect.element(page.getByRole("tab", { name: "外部入力" })).toBeInTheDocument();
  });
  ...
  test("ラジオタブを選択すると Radiko/らじる 局リストが表示される", async () => {
    ...
    await expect.element(page.getByTestId("radiru-station")).toBeInTheDocument();
  });
  ...
  test("外部入力タブで mediaStream なしの場合 ScreenShare と ExternalInput が表示される", async () => {
    ...
    await expect.element(page.getByTestId("external-input")).toBeInTheDocument();
  });
  ...
  test("外部入力タブで mediaStream ありの場合 DisconnectInput が表示される", async () => {
    ...
    await expect.element(page.getByTestId("disconnect-input")).toBeInTheDocument();
  });
