...
import { currentRadioAtom, customFrequencyAreaAtom, radioChannelsByAreaAtom } from "@/atoms/radio";
  ...
  const handleFrequencyChange = (nextType: RadioType, freq: number) => {
    setCustomFreqList((stations) =>
      stations.find((s) => s.id === stationId)
        ? [...stations.filter((s) => s.id !== stationId), { id: stationId, type: nextType, freq }]
        : [...stations, { id: stationId, type: nextType, freq }],
    );
    setCurrentRadio((prev) => {
      if (!prev || prev.source !== "radiko" || prev.id !== stationId) return prev;
    ...
    <ContextMenuContent className="min-w-48">
        ...
        <ContextMenuSubContent>
          ...
          >
            {station?.frequencies_fm?.map((area) => (
              <ContextMenuRadioItem key={area.area.toString()} value={`FM-${area.frequency}`}>
                {area.frequency.toFixed(1)}MHz ({area.area.join(", ")})
              </ContextMenuRadioItem>
            ))}
            {station?.frequencies_am?.map((area) => (
              <ContextMenuRadioItem key={area.area.toString()} value={`AM-${area.frequency}`}>
      ...
      <ContextMenuSub>
        ...
        <ContextMenuSubContent>
          {CHANNEL_NUMS.map((ch) => {
            const bandKey = type === "AM" ? "am" : "fm";
            const existing = areaId ? channelsByArea[areaId]?.[bandKey]?.[ch] : undefined;
            const isSelf = existing?.stationId === stationId;
            return (
              <ContextMenuItem key={ch} onClick={() => handleAssignChannel(ch)}>
                <span className={isSelf ? "text-neutral-100 font-semibold" : ""}>CH{ch}</span>
                {existing && !isSelf && (
                  <span className="ml-auto max-w-24 truncate text-xs text-muted-foreground">
                    {existing.stationName}
                  </span>
                )}
                {isSelf && <span className="ml-auto text-xs text-neutral-400">✓</span>}
              </ContextMenuItem>
            );
          })}
        </ContextMenuSubContent>
