...
describe("ExternalInput", () => {
  ...
  test("デバイス一覧が表示される", async () => {
    ...
    // useQuery のデータ取得を待つ
    await expect.element(page.getByText("Default - Built-in Microphone")).toBeInTheDocument();
    await expect.element(page.getByText("External USB Microphone")).toBeInTheDocument();
  });
  ...
  test("デバイスクリックで connect が呼ばれる", async () => {
    ...
    await page.getByRole("button", { name: "Default - Built-in Microphone" }).click();
