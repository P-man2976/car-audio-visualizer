...
export function RadioStation({ name, id, logo }: RadikoStation) {
  ...
  const isSelected =
    currentSrc === "radio" && currentRadio?.source === "radiko" && currentRadio.id === id;
        ...
        >
          <div
            className={cn(
              ...
              size === "lg" && "w-24",
              isSelected ? "bg-gray-300" : "bg-gray-500/50 group-hover:bg-gray-400/50",
            )}
          >
            {logo?.[0] ? <img src={logo[0]} alt={name} /> : <span>{name.slice(0, 2)}</span>}
          </div>
          {size === "lg" ? (
            <>
