...
export function ExplorerDialog({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (isOpen && !autoOpenedRef.current && savedHandles.length > 0 && stack.length === 0) {
  ...
  const collectFileHandles = async (files: SelectedFile[]): Promise<FileSystemFileHandle[]> => {
  ...
  const collectFromDir = async (
    ...
    for (const entry of entries) {
      if (entry.handle.kind === "directory") {
        results.push(...(await collectFromDir(entry.handle as FileSystemDirectoryHandle)));
      } else {
  ...
  const handleFallbackChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const songs = (await Promise.all(files.map((f) => fileToSong(f, audioElement)))).filter(
      (s) => s !== undefined,
    );
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (files: SelectedFile[]) => {
      ...
      const songs = pairs.map((p) => p.song).filter((s): s is Song => s !== undefined);
  ...
  if (!hasFSAPI) {
    return (
      <>
        ...
        />
        <span className="contents" onClick={() => fallbackInputRef.current?.click()}>
          {children}
        </span>
      </>
    );
      ...
      <DialogContent className="flex flex-col h-[calc(100dvh-8rem)] sm:max-w-4xl">
        ...
        <div className="flex gap-1 overflow-auto h-full">
          ...
          <div className="basis-1/4 w-full flex flex-col gap-2 pt-2">
            <Button variant="ghost" className="justify-start gap-2" onClick={handleSelectRoot}>
              <LuFolderOpen />
            ...
            {savedHandles.length > 0 && (
              <div className="flex flex-col gap-0.5">
                <span className="text-xs text-muted-foreground px-2 pt-1">保存済みフォルダ</span>
                {savedHandles.map((h) => (
                  <Button
                    key={h.name}
                    variant="ghost"
