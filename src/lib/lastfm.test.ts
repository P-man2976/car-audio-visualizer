...
describe("getSession", () => {
  ...
  test("HTTP エラー時に例外を投げる", async () => {
    ...
    await expect(getSession("bad-token")).rejects.toThrow("Last.fm auth.getSession failed: 403");
  });
  ...
  test("API エラーレスポンス時に例外を投げる", async () => {
    ...
    await expect(getSession("expired-token")).rejects.toThrow("Last.fm error 4: Invalid token");
  });
});
