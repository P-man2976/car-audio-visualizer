...
describe("buildDisplayString", () => {
	...
	it("returns FM display for FM radio without frequency", () => {
		expect(buildDisplayString("radio", { type: "FM", source: "radiko", id: "FMJ", name: "J-WAVE" }, 0)).toBe("F1- --.- MHz");
	});

	it("returns FM display for FM radio with frequency", () => {
		expect(buildDisplayString("radio", { type: "FM", source: "radiko", id: "FMJ", name: "J-WAVE", frequency: 81.3 }, 0)).toBe("F1- 81.3 MHz");
	});

	it("returns AM display for AM radio without frequency (radiru)", () => {
		expect(buildDisplayString("radio", { type: "AM", source: "radiru", url: "https://example.com", name: "ラジオ第一" }, 0)).toBe("A1- ---- kHz");
	});

	it("returns AM display for AM radio with frequency", () => {
		expect(buildDisplayString("radio", { type: "AM", source: "radiko", id: "TBS", name: "TBSラジオ", frequency: 954 }, 0)).toBe("A1-  954 kHz");
	});
