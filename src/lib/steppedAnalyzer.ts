...
export class SteppedAnalyzer {
  ...
  constructor(interval = 200, fallSpeed = 1.0, peakHoldTime = 500, peakFallSpeed = 0.3) {
   ...
   */
  update(getBars: () => AnalyzerBarData[], now: number): AnalyzerBarData[] | null {
    ...
    if (elapsed >= this.interval || this.targetValues.length === 0) {
      ...
      this.targetValues = currentBars.map((b) => [...b.value] as [number, number]);
