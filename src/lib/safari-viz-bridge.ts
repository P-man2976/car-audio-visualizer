...
import { type BufferAppendingData, type BufferCodecsData, Events } from "hls.js";
...
export class SafariVizBridge {
  ...
  constructor(analyzer: AudioMotionAnalyzer) {
    ...
    this.bridgeAnalyser.smoothingTimeConstant = origAnalyser.smoothingTimeConstant;
   ...
   */
  async attachFile(blobUrl: string, audioElement: HTMLAudioElement): Promise<void> {
    this.detach();
    this.detachFile();
   ...
   */
  private onBufferCodecs = (_event: Events.BUFFER_CODECS, data: BufferCodecsData): void => {
  ...
  private onBufferAppending = (
    ...
    const combined = new Uint8Array(this.initSegment.length + segmentData.length);
   ...
   */
  private scheduleBuffer(audioBuffer: AudioBuffer, mediaStartTime: number): void {
    const source = this.audioCtx.createBufferSource();
