...
describe("DEFAULT_AM_FILTER_SETTINGS", () => {
  ...
  it("スピーカー共振周波数が可聴域内である", () => {
    expect(DEFAULT_AM_FILTER_SETTINGS.speakerResonanceFreq).toBeGreaterThanOrEqual(20);
    expect(DEFAULT_AM_FILTER_SETTINGS.speakerResonanceFreq).toBeLessThanOrEqual(20000);
  });

  it("スピーカー共振ゲインが 0 以上である", () => {
    expect(DEFAULT_AM_FILTER_SETTINGS.speakerResonanceGain).toBeGreaterThanOrEqual(0);
  });
...
describe("amFilterSettingsAtom", () => {
  ...
  it("updater 関数で set した値が read で反映される", () => {
    ...
    // 他のフィールドはデフォルトのまま
    expect(store.get(amFilterSettingsAtom).hpfFreq).toBe(DEFAULT_AM_FILTER_SETTINGS.hpfFreq);
  });
});
