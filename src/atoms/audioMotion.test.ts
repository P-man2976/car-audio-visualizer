...
describe("DEFAULT_AUDIO_MOTION_SETTINGS", () => {
  ...
  it("smoothingTimeConstant は 0 以上 1 未満", () => {
    expect(DEFAULT_AUDIO_MOTION_SETTINGS.smoothingTimeConstant).toBeGreaterThanOrEqual(0);
...
describe("FFT_SIZE_OPTIONS", () => {
  ...
  it("昇順にソートされている", () => {
    for (let i = 1; i < FFT_SIZE_OPTIONS.length; i++) {
      expect(FFT_SIZE_OPTIONS[i]).toBeGreaterThan(FFT_SIZE_OPTIONS[i - 1] as number);
    }
  });
...
describe("AUDIO_MOTION_MODE_LABELS", () => {
  it("モード 0〜10 のラベルがすべて存在する", () => {
    for (let m = 0; m <= 10; m++) {
      expect(AUDIO_MOTION_MODE_LABELS[m as keyof typeof AUDIO_MOTION_MODE_LABELS]).toBeTruthy();
    }
  });

  it("各ラベルはモード番号を含む文字列である", () => {
    for (let m = 0; m <= 10; m++) {
      const label = AUDIO_MOTION_MODE_LABELS[m as keyof typeof AUDIO_MOTION_MODE_LABELS];
...
describe("WEIGHTING_FILTER_LABELS", () => {
  ...
  it("すべての有効なフィルター名のラベルが存在する", () => {
    for (const key of EXPECTED_KEYS) {
      expect(WEIGHTING_FILTER_LABELS[key as keyof typeof WEIGHTING_FILTER_LABELS]).toBeTruthy();
    }
  });
